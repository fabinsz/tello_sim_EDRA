...
class Simulator():
    ...
    def plot_field(self):
      ...
      for i, (tx, ty) in enumerate(self.treasures):
          if i in self.visited_treasures:
              self.ax.plot(tx, ty, 'gx', markersize=15, label="Tesouro Coletado" 
                         if 'Tesouro Coletado' not in self.ax.get_legend_handles_labels()[1] else "")
          else:
              self.ax.plot(tx, ty, 'g*', markersize=15, label="Tesouro" 
                         if 'Tesouro' not in self.ax.get_legend_handles_labels()[1] else "")
